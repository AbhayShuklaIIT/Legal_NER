...
def get_labels_from_ctx(contexts,label_to_int_dict,model, combined, use_cache):
    global model_cache
    model_probs = []
    for l in label_to_int_dict.keys():
        probs = []
        if len(contexts) == 0:continue
        if combined == 1:
        ##########
            com_ctx = ". ".join(contexts)
            ip = l + "[SEP]" + com_ctx
            with torch.no_grad():
                if use_cache == 1 and ip in model_cache.keys():
                    p = model_cache[ip]
                else:
                    p = model(ip)
                    if ip not in model_cache.keys():model_cache[ip] = p
                if use_cache == 2:
                    model_cache[ip] = p

            model_probs.append(p.to("cpu").item())
        else:
        #############
            for ctx in contexts:
                ip = l + "[SEP]" + ctx
                with torch.no_grad():
                    ...
                    if use_cache == 2:
                        model_cache[ip] = p
                probs.append(p.to("cpu").item())
            model_probs.append(sum(probs)/len(probs))
    m_i = argmax(model_probs)
    model_probs = [0 for i in range(len(model_probs))]
    model_probs[m_i] = 1
#     print(model_probs)
...
def get_parent_to_child_using_similarity(n):
    ...
    for n_i in n:
        ...
        for n_j in n:
            if n_i == n_j:continue
            if get_similarity_1(n_i,n_j) > thres:
...
def evaluate_post_v1(path, label_to_int_dict, model, combined, use_cache, removed=[]):
    ...
    for i in range(len(nes)):
        ...
        if fl[i] != "":
            pred_ints = get_labels_from_ctx(cs, label_to_int_dict, model, combined, use_cache)
            freq = fl[i]
        else:
...
def print_result_v1(model_name, combined, device, cache_file, use_cache):
    global model_cache
    if use_cache == 2 or use_cache == 1: model_cache = {}
...
#     print("Gold" , roles_to_ne_gs)
    ...
    for i in range(len(nes)):
        ...
        pred = get_labels_from_ctx(j, label_to_int_dict, model, combined, use_cache)
...
def print_result_v2(model_name, combined, device, cache_file, use_cache, strict):
    global model_cache
